...
test.describe('Send client info functionality', () => {
    ...
    test('should successfully send info about client without checkbox', async ({page}) => {
        await expect(page).toHaveURL('http://localhost:5173/#/profile');
        ...
        await page.fill('input[placeholder="Контактное лицо"]', Math.random().toString(36).substring(2, 10));
        await page.fill('input[placeholder="Телефон контактного лица"]', Math.random().toString(36).substring(2, 10));
        await page.fill('input[placeholder="ФИО руководителя"]', Math.random().toString(36).substring(2, 10));
        await page.fill('input[placeholder="Телефон руководителя"]', Math.random().toString(36).substring(2, 10));
